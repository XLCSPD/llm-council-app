...
export function addReasoningSection(doc: jsPDF, data: ReasoningSectionData, startY: number): number {
  ...
  for (const model of data.runModels) {
    ...
    // Response content - render with markdown formatting
    const responseContent = reasoningOutput.content || 'No response';

    // Render markdown content with visual hierarchy
    y = renderMarkdownToPdf(doc, responseContent, {
      startX: margin + 4,
      startY: y,
      maxWidth: contentWidth - 8,
    });

    y += PDF_SPACING.paragraphGap + 4;
  }
